...
function FireworksOverlay({ active, onDone }: { active: boolean; onDone: () => void }) {
  ...
  const loadTask = async () => {
    try {
      ...
      if (taskData?.topic_id) {
        const topicData = await contentAPI.getTopicById(taskData.topic_id);
        setTopic(topicData);
      }
                      ...
                      >
                        <div className='bg-slate-800/50 rounded-lg p-4 border border-slate-700/30'>
                          <p className='text-slate-200 whitespace-pre-wrap'>
                            {task.official_solution || 'No solution provided'}
                          </p>
                        </div>
                      </motion.div>
                    )}
                      ...
                      >
                        <div className='bg-slate-800/50 rounded-lg p-4 border border-slate-700/30 text-center'>
                          <p className='text-xl font-semibold text-emerald-400'>
                            {task.correct_answer || 'No answer provided'}
                          </p>
                        </div>
                      </motion.div>
                    )}
