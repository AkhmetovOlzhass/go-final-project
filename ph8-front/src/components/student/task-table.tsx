...
export function TaskTable({ tasks, topics, onViewTask, userSolutions = [] }: TaskTableProps) {
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    const matchesTopic = selectedTopic === 'all' || task.topic_id === selectedTopic;
  ...
  const getTopicClass = (topicId: string) => {
    const schoolClass = topics.find((topic) => topic.id === topicId)?.school_class;
  ...
  const tasksToRender =
    classSort === 'none'
      ...
      : [...filteredTasks].sort((a, b) => {
          const ca = getClassNumber(a.topic_id);
          const cb = getClassNumber(b.topic_id);
                ...
                <td className='px-6 py-5 whitespace-nowrap'>
                  <Badge
                    className={`${getDifficultyColor(
                      task.difficulty,
                    )} text-xs font-semibold px-3 py-1.5 transition-all duration-200 border`}
                  >
                    {task.difficulty}
                  </Badge>
                </td>
